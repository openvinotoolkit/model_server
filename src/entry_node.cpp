...
Status EntryNode::fetchResults(NodeSession& nodeSession, SessionResults& nodeSessionOutputs) {
    ...
    std::pair<NodeSessionMetadata, BlobMap> metaOutputsPair{nodeSession.getNodeSessionMetadata(), std::move(outputs)};
    SPDLOG_ERROR("Session results size:{}", nodeSessionOutputs.size());
    nodeSessionOutputs.emplace(nodeSession.getSessionKey(), std::move(metaOutputsPair));
    SPDLOG_ERROR("Session results size:{}", nodeSessionOutputs.size());
    // TODO error check
    return StatusCode::OK;
}
