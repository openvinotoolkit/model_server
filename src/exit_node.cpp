...
Status ExitNode::fetchResults(NodeSession& nodeSession, SessionResults& nodeSessionOutputs) {
    auto& exitNodeSession = dynamic_cast<ExitNodeSession&>(nodeSession);
    return this->fetchResults(exitNodeSession.getInputBlobs());
}

/*Status ExitNode::execute(PipelineEventQueue& notifyEndQueue) {
    notifyEndQueue.push(NodeSessionKeyPair(*this, "-1"));
    return StatusCode::OK;  // TODO remove
}*/
