...
const std::map<const StatusCode, const std::string> Status::statusMessageMap = {
    ...
    {StatusCode::PIPELINE_GATHER_FROM_NOT_EXISTING_NODE, "Gather node refers to not existing node"},
    {StatusCode::PIPELINE_GATHER_FROM_NOT_DEMULTIPLEXER, "Gather node refers to node that isn't demultiplexer"},
    {StatusCode::PIPELINE_GATHER_FROM_ENTRY_NODE, "Gathering from entry node is not allowed"},
