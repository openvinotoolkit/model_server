...
const std::unordered_map<const StatusCode, const std::string> Status::statusMessageMap = {
    ...
    {StatusCode::NODE_LIBRARY_OUTPUTS_CORRUPTED, "Custom node library has corrupted outputs handle"},
    {StatusCode::NODE_LIBRARY_OUTPUTS_CORRUPTED_COUNT, "Custom node library has corrupted number of outputs"},
    {StatusCode::NODE_LIBRARY_INVALID_PRECISION, "Custom node produced blob precision is unspecified"},
    {StatusCode::NODE_LIBRARY_INVALID_SHAPE, "Custom node produced blob shape does not match"},
