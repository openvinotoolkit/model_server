...
#elif _WIN32
            ("cache_dir",
                "Overrides model cache directory. By default cache files are saved into c:\\Intel\\openvino_cache if the directory is present. When enabled, first model load will produce cache files.",
                cxxopts::value<std::string>(),
                "CACHE_DIR")
#endif
            ("metrics_enable",
