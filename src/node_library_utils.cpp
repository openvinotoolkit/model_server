...
Status createTensorInfoMap(struct CustomNodeTensorInfo* info, int infoCount, std::map<std::string, std::shared_ptr<TensorInfo>>& out, NodeLibraryExecutor& nodeLibraryExecutor, void* customNodeLibraryInternalManager) {
    if (info == nullptr) {
        return StatusCode::NODE_LIBRARY_OUTPUTS_CORRUPTED;
    }
    if (infoCount <= 0) {
        nodeLibraryExecutor.release(info, customNodeLibraryInternalManager);
    ...
    for (int i = 0; i < infoCount; i++) {
        ...
        if (info[i].dimsCount == 0) {
            nodeLibraryExecutor.release(info[i].dims, customNodeLibraryInternalManager);
            continue;
        }
