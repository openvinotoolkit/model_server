...
template <>
const std::vector<int64_t>& RequestShapeInfo<InferenceTensor, std::vector<int64_t>>::getShape() {  // TODO: Declare CAPIShapeType
...
public:
    ...
    Status checkBinaryBatchSizeMismatch(const InputTensorType& proto, const Dimension& servableBatchSize, Status& finalStatus, Mode batchingMode, Mode shapeMode, int32_t inputBatchSize) const;
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::validateRequestCoherency() const {
    return StatusCode::OK;
}
...
template <>
const std::string& RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::getCurrentlyValidatedInputName() const {
    return *currentlyValidatedName;
}
...
template <>
const InferenceTensor& RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::getInputFromIt(const InferenceTensor* const& it) const {
    return *it;
}
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::validateNumberOfInputs() const {
    size_t expectedNumberOfInputs = inputsInfo.size();
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::validateAndGetInput(const InferenceRequest& request, const std::string& name, const InferenceTensor*& it, size_t& bufferId) {
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::validateNumberOfBinaryInputShapeDimensions(const InferenceTensor& tensor) const {
...
template <>
Status RequestValidator<TFSRequestType, TFSInputTensorType, TFSInputTensorIteratorType, TFSShapeType>::checkBinaryBatchSizeMismatch(const TFSInputTensorType& proto, const Dimension& servableBatchSize, Status& finalStatus, Mode batchingMode, Mode shapeMode, int32_t inputBatchSize) const {
    RequestShapeInfo<TFSInputTensorType, TFSShapeType> rsi(proto);
    if (inputBatchSize <= 0) {
        std::stringstream ss;
...
template <>
Status RequestValidator<KFSRequest, KFSTensorInputProto, KFSInputTensorIteratorType, KFSShapeType>::checkBinaryBatchSizeMismatch(const KFSTensorInputProto& proto, const Dimension& servableBatchSize, Status& finalStatus, Mode batchingMode, Mode shapeMode, int32_t inputBatchSize) const {
    RequestShapeInfo<KFSTensorInputProto, KFSShapeType> rsi(proto);
    if (inputBatchSize <= 0) {
        std::stringstream ss;
        ss << "Batch size must be positive; input name: " << getCurrentlyValidatedInputName();
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::checkBinaryBatchSizeMismatch(const InferenceTensor& tensor, const Dimension& servableBatchSize, Status& finalStatus, Mode batchingMode, Mode shapeMode, int32_t inputBatchSize) const {
    RequestShapeInfo<InferenceTensor, CAPIShapeType> rsi(tensor);
    if (rsi.getDim(0) <= 0) {
        std::stringstream ss;
        ss << "Batch size must be positive; input name: " << getCurrentlyValidatedInputName();
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::validateTensorContent(const InferenceTensor& tensor, ovms::Precision expectedPrecision, size_t bufferId) const {
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::validateNumberOfShapeDimensions(const ovms::TensorInfo& inputInfo, const InferenceTensor& tensor) const {
    // Network and request must have the same number of shape dimensions, higher than 0
    const auto& shape = inputInfo.getShape();
...
template <>
Status RequestValidator<ovms::InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>::validatePrecision(const ovms::TensorInfo& inputInfo, const InferenceTensor& tensor) const {
...
Status validate(const InferenceRequest& request, const tensor_map_t& inputsInfo, const std::string& servableName, const model_version_t servableVersion, const std::set<std::string>& optionalAllowedInputNames, const Mode batchingMode, const shapes_info_map_t& shapeInfo) {
    OVMS_PROFILE_FUNCTION();
    return RequestValidator<InferenceRequest, InferenceTensor, const InferenceTensor*, CAPIShapeType>(request, inputsInfo, servableName, servableVersion, optionalAllowedInputNames, batchingMode, shapeInfo).validate();
}
}  // namespace request_validation_utils
}  // namespace ovms
