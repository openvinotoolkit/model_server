...
Status DLNode::execute(session_key_t sessionKey, PipelineEventQueue& notifyEndQueue) {
    auto& nodeSession = getNodeSession(sessionKey);                   // TODO error check?
    auto& dlNodeSession = dynamic_cast<DLNodeSession&>(nodeSession);  // TODO error check dynamic_cast
...
Status DLNode::fetchResults(NodeSession& nodeSession, SessionResults& nodeSessionOutputs) {
    auto& dlNodeSession = dynamic_cast<DLNodeSession&>(nodeSession);
    const auto& sessionMetadata = nodeSession.getNodeSessionMetadata();
    std::pair<NodeSessionMetadata, BlobMap> sessionResults{sessionMetadata, {}};
    auto it = nodeSessionOutputs.emplace(sessionMetadata.getSessionKey(), std::move(sessionResults));  // TODO error check but if it does not succeed then error was earlier
...
Status DLNode::fetchResults(BlobMap& outputs, InferenceEngine::InferRequest& inferRequest, ModelInstance& model, session_key_t sessionKey) {
    ...
    // TODO replace streamId with sessionId
    SPDLOG_DEBUG("[Node: {}] Waiting for infer request to finish", getName(), sessionKey);
    auto ov_status = inferRequest.Wait(InferenceEngine::IInferRequest::RESULT_READY);
    SPDLOG_DEBUG("[Node: {}] Infer request with streamId: {} finished", getName(), sessionKey);

    dynamic_cast<DLNodeSession&>(this->getNodeSession(sessionKey)).clearInputs();
        ...
        for (const auto& pair : node.get().getMappingByDependency(*this)) {
            ...
            try {
                ...
                SPDLOG_DEBUG("[Node: {}] Getting blob from model: {}, inferRequestStreamId: {}, blobName: {}",
                    getName(), modelName, sessionKey, realModelOutputName);
