...
namespace ovms {
RerankModel::RerankModel(const std::filesystem::path& model_dir,
...
void RerankModel::prepareInferenceRequestsQueue(const uint32_t& numberOfParallelInferRequests) {
    inferRequestsQueue = std::make_unique<OVInferRequestsQueue>(compiledModel, numberOfParallelInferRequests);
}

RerankServable::RerankServable(const ::mediapipe::CalculatorGraphConfig::Node& graphNodeConfig) {
    ...
    embeddings = std::make_shared<RerankModel>(std::filesystem::path(model_dir), device, embeddingsPoperties);
}
