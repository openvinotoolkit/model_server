...
public:
    ...
    static const std::string GENAI_EMBEDDINGS_SESSION_SIDE_PACKET_TAG;
    ...
    Status infer(const RequestType* request, ResponseType* response, ExecutionContext executionContext) {
        ...
        inputSidePackets[GENAI_EMBEDDINGS_SESSION_SIDE_PACKET_TAG] = mediapipe::MakePacket<GenaiEmbeddingsServableMap>(this->sidePacketMaps.genaiEmbeddingsServableMap).At(STARTING_TIMESTAMP);
