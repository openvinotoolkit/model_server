...
public:
    ...
    MediapipeGraphExecutor(const std::string& name, const std::string& version, const ::mediapipe::CalculatorGraphConfig& config,
        ...
        const std::shared_ptr<GraphSidePackets>& sidePacketMaps,
        PythonBackend* pythonBackend,
...
#if (PYTHON_DISABLE == 0)
<<<<<<< HEAD
        inputSidePackets[PYTHON_SESSION_SIDE_PACKET_NAME] = mediapipe::MakePacket<PythonNodeResourcesMap>(this->sidePacketMaps.pythonNodeResourcesMap).At(STARTING_TIMESTAMP);
=======
...
>>>>>>> 99ac08c4b (Check graph pool)
#endif
        inputSidePackets[LLM_SESSION_SIDE_PACKET_NAME] = mediapipe::MakePacket<GenAiServableMap>(this->sidePacketMaps.genAiServableMap).At(STARTING_TIMESTAMP);
        inputSidePackets[IMAGE_GEN_SESSION_SIDE_PACKET_NAME] = mediapipe::MakePacket<ImageGenerationPipelinesMap>(this->sidePacketMaps.imageGenPipelinesMap).At(STARTING_TIMESTAMP);
        inputSidePackets[EMBEDDINGS_SESSION_SIDE_PACKET_NAME] = mediapipe::MakePacket<EmbeddingsServableMap>(this->sidePacketMaps.embeddingsServableMap).At(STARTING_TIMESTAMP);

        inputSidePackets[RERANK_SESSION_SIDE_PACKET_NAME] = mediapipe::MakePacket<RerankServableMap>(this->sidePacketMaps.rerankServableMap).At(STARTING_TIMESTAMP);
