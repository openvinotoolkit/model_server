...
bool copy_images_into_output(struct CustomNodeTensor* output, const std::vector<cv::Rect>& boxes, const cv::Mat& originalImage, int targetImageHeight, int targetImageWidth, const std::string& targetImageLayout, bool convertToGrayScale, void* customNodeLibraryInternalManager) {
    ...
    if (!get_preallocated_buffer<float>(internalManager, &buffer, OUTPUT_IMAGES_TENSOR_NAME))
        return false;
    ...
    if (!get_preallocated_buffer<u_int64_t>(internalManager, &dims, OUTPUT_IMAGES_DIMS_NAME))
        return false;
    output->dimsCount = 5;
    output->dims = dims;
...
bool copy_coordinates_into_output(struct CustomNodeTensor* output, const std::vector<cv::Vec4f>& detections, void* customNodeLibraryInternalManager) {
    ...
    if (!get_preallocated_buffer<int32_t>(internalManager, &buffer, OUTPUT_COORDINATES_TENSOR_NAME))
        return false;
    ...
    u_int64_t* dims = nullptr;
    if (!get_preallocated_buffer<u_int64_t>(internalManager, &dims, OUTPUT_COORDINATES_DIMS_NAME))
        return false;
    output->dimsCount = 3;
    output->dims = dims;
    output->dims[0] = outputBatch;
...
bool copy_confidences_into_output(struct CustomNodeTensor* output, const std::vector<float>& confidences, void* customNodeLibraryInternalManager) {
    ...
    float* buffer = nullptr;
    if (!get_preallocated_buffer<float>(internalManager, &buffer, OUTPUT_CONFIDENCES_TENSOR_NAME))
        return false;
    ...
    u_int64_t* dims = nullptr;
    if (!get_preallocated_buffer<u_int64_t>(internalManager, &dims, OUTPUT_CONFIDENCES_DIMS_NAME))
        return false;
    output->dimsCount = 3;
    output->dims = dims;
    output->dims[0] = outputBatch;
