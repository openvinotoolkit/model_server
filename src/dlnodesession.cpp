...
namespace ovms {
DLNodeSession::DLNodeSession(const NodeSessionMetadata& metadata, const std::string& nodeName, uint32_t inputsCount, const ModelManager& manager, const std::string& modelName, model_version_t modelVersion) :
...
Status DLNodeSession::requestExecuteRequiredResources() {  // TODO remove corresponding function in dlnode
    Status status = StatusCode::OK;
    ModelManager& manager = const_cast<ModelManager&>(modelManager);
    status = manager.getModelInstance(
        modelName,
        modelVersion,
        this->model,
    ...
    if (!status.ok()) {
        SPDLOG_LOGGER_DEBUG(dag_executor_logger, "Getting modelInstance failed for node: {} with: {}", getName(), status.string());  // TODO
        return status;
    }
