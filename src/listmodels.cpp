...
// write implementation for listServables function
#include <filesystem>
#include <fstream>
#include <iostream>
...
namespace ovms {
    ...
    // that will return a map of servables from current directory
    // it will recursively search for directory and
    // -> if if contains graph.pbtxt file it will be added to the map as SERVABLE_TYPE_MEDIAPIPEGRAPH
    // -> if it contains version directory it will check if it contains any of the extensions from OV_MODEL_FILES_EXTENSIONS
    // -> if it contains version directory it will check if it contains any of the extensions from ONNX_MODEL_FILES_EXTENSIONS
    // note that if directory contains already graph.pbtxt it won't check if subdirectories of that directory contains any of the extensions nor graph.pbtxt
    // now implement function

// write a function that based on the path, int depth creates string containing filename and up to depth levelds of directories above it
// for example if path is /home/user/dir1/dir2/file.txt and depth is 2 it will return dir1/dir2/file.txt
// if depth is 0 it will return file.txt
// if depth is 1 it will return dir2/file.txt
// if depth is 2 it will return dir1/dir2/file.txt
// if depth is more than the number of directories it will throw runtime_error
std::string getPartialPath(const std::filesystem::path& path, int depth) {
    std::string partialPath = path.filename().string();
    std::filesystem::path parentPath = path.parent_path();
    SPDLOG_ERROR("ER:{}", partialPath);
    for (int i = 0; i < depth; ++i) {
        if (parentPath.empty() || parentPath == parentPath.root_path()) {
            SPDLOG_ERROR("ER:{}, {}", partialPath, i);
