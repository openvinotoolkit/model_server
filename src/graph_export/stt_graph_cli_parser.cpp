...
//*****************************************************************************
#include "stt_graph_cli_parser.hpp"
...
void SpeechToTextGraphCLIParser::prepare(OvmsServerMode serverMode, HFSettingsImpl& hfSettings, const std::string& modelName) {
    ...
    speechToTextGraphSettings.targetDevice = hfSettings.exportSettings.targetDevice;
    if (modelName != "") {
        speechToTextGraphSettings.modelName = modelName;
    } else {
        speechToTextGraphSettings.modelName = hfSettings.sourceModel;
    }
    if (nullptr == result) {
        ...
        speechToTextGraphSettings.numStreams = result->operator[]("num_streams").as<uint32_t>();
    }
