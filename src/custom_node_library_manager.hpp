...
class CustomNodeLibraryManager {
    std::unordered_map<std::string, std::shared_ptr<NodeLibraryExecutor>> libraries;

public:
    Status loadLibrary(const std::string& name, const std::string& basePath);
    Status getLibrary(const std::string& name, std::shared_ptr<NodeLibraryExecutor>& library) const;
