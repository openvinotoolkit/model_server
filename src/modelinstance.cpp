...
Status applyLayoutConfiguration(const ModelConfig& config, std::shared_ptr<ov::Model>& model, const std::string& modelName, model_version_t modelVersion) {
    ...
    for (const ov::Output<ov::Node>& input : model->inputs()) {
        try {
            ...
            std::string mapped_name = config.getMappingInputByKey(name).empty() ? name : config.getMappingInputByKey(name);
            ...
            } else if (config.getLayouts().count(mapped_name) > 0) {
                auto& layout = config.getLayouts().at(mapped_name);
    ...
    for (const ov::Output<ov::Node>& output : model->outputs()) {
        try {
            ...
            std::string mapped_name = config.getMappingOutputByKey(name).empty() ? name : config.getMappingOutputByKey(name);

            if (config.getLayouts().count(mapped_name) > 0) {
                auto& layout = config.getLayouts().at(mapped_name);
