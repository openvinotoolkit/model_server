...
node {
  ...
  output_stream: "RESPONSE:out"
    ...
    MediapipeGraphExecutor executor{
        ...
        {"in"}, {"out"}, graphSidePackets, nullptr, this->reporter.get(), std::move(guard)};
...
node {
  ...
  output_stream: "out3"
    ...
    MediapipeGraphExecutor executor{
        ...
        {"in1", "in2", "in3"},
        {"out1", "out2", "out3"},
        graphSidePackets, nullptr, this->reporter.get(), std::move(guard)};
...
node {
  ...
  output_stream: "out"
    ...
    MediapipeGraphExecutor executor{
        ...
        {{"in", mediapipe_packet_type_enum::OVTENSOR}},
        {{"out", mediapipe_packet_type_enum::OVTENSOR}},
        {"in"}, {"wrong_name"}, graphSidePackets, nullptr, this->reporter.get(), std::move(guard)};  // cannot install observer due to wrong output name (should never happen due to validation)
