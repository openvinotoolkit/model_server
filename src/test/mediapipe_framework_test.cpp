...
TEST_F(MediapipeNegativeFrameworkTest, NoOutputPacketProduced) {
    SetUpServer(getWindowsFullPathForSrcTest("/ovms/src/test/mediapipe/negative/config_no_calc_output_stream.json"));
...
TEST_F(MediapipeNegativeFrameworkTest, ExceptionDuringProcess) {
    ...
    SetUpServer(getWindowsFullPathForSrcTest("/ovms/src/test/mediapipe/negative/config_exception_during_process.json"));
    const ovms::Module* grpcModule = server.getModule(ovms::GRPC_SERVER_MODULE_NAME);
    KFSInferenceServiceImpl& impl = dynamic_cast<const ovms::GRPCServerModule*>(grpcModule)->getKFSGrpcImpl();
    ::KFSRequest request;
    ::KFSResponse response;
...
TEST_F(MediapipeNegativeFrameworkTest, ExceptionDuringGetContract) {
    SetUpServer(getWindowsFullPathForSrcTest("/ovms/src/test/mediapipe/negative/config_exception_during_getcontract.json"));
    const ovms::Module* grpcModule = server.getModule(ovms::GRPC_SERVER_MODULE_NAME);
    KFSInferenceServiceImpl& impl = dynamic_cast<const ovms::GRPCServerModule*>(grpcModule)->getKFSGrpcImpl();
    ::KFSRequest request;
    ::KFSResponse response;
...
TEST_F(MediapipeNegativeFrameworkTest, ExceptionDuringGetOpen) {
    GTEST_SKIP() << "Terminate called otherwise";
    SetUpServer(getWindowsFullPathForSrcTest("/ovms/src/test/mediapipe/negative/config_exception_during_open.json"));
    const ovms::Module* grpcModule = server.getModule(ovms::GRPC_SERVER_MODULE_NAME);
    KFSInferenceServiceImpl& impl = dynamic_cast<const ovms::GRPCServerModule*>(grpcModule)->getKFSGrpcImpl();
    ::KFSRequest request;
    ::KFSResponse response;
...
TEST_F(MediapipeNegativeFrameworkTest, ExceptionDuringClose) {
    GTEST_SKIP() << "Terminate called otherwise";
    SetUpServer(getWindowsFullPathForSrcTest("/ovms/src/test/mediapipe/negative/config_exception_during_close.json"));
    const ovms::Module* grpcModule = server.getModule(ovms::GRPC_SERVER_MODULE_NAME);
    KFSInferenceServiceImpl& impl = dynamic_cast<const ovms::GRPCServerModule*>(grpcModule)->getKFSGrpcImpl();
    ::KFSRequest request;
    ::KFSResponse response;
