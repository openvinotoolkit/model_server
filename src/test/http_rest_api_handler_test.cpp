...
TEST_F(ConfigReload, startWith1DummyThenAddVersion) {
    ...
    std::filesystem::copy(getWindowsFullPathForSrcTest("/ovms/src/test/dummy"), "/tmp/dummy", std::filesystem::copy_options::recursive);
 ...
 "model_version_status": [
  {
   ...
   "status": {
    ...
    std::filesystem::copy(getWindowsFullPathForSrcTest("/ovms/src/test/dummy/1"), getWindowsFullPathForTmp("/tmp/dummy/2"), std::filesystem::copy_options::recursive);
 ...
 "model_version_status": [
  ...
  },
  {
   ...
   "status": {
    ...
    EXPECT_EQ(status, ovms::StatusCode::OK_RELOADED);
    std::filesystem::remove_all(getWindowsFullPathForTmp("/tmp/dummy"));
}

TEST_F(ConfigReload, startWithMissingXmlThenAddAndReload) {
    ovms::Server& ovmsServer = ovms::Server::instance();
    std::filesystem::remove_all(getWindowsFullPathForTmp("/tmp/dummy"));
    std::filesystem::create_directory(getWindowsFullPathForTmp("/tmp/dummy"));
    std::filesystem::create_directory(getWindowsFullPathForTmp("/tmp/dummy/1"));
    std::filesystem::copy(getWindowsFullPathForSrcTest("/ovms/src/test/dummy/1/dummy.bin"), getWindowsFullPathForTmp("/tmp/dummy/1/dummy.bin"), std::filesystem::copy_options::recursive);
    TestHelper1 t(*this, configWith1DummyInTmp);
    auto handler = ovms::HttpRestApiHandler(ovmsServer, 10);
    ...
    std::filesystem::copy(getWindowsFullPathForSrcTest("/ovms/src/test/dummy/1/dummy.xml"), getWindowsFullPathForTmp("/tmp/dummy/1/dummy.xml"), std::filesystem::copy_options::recursive);
