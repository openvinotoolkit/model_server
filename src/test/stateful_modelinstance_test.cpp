...
TEST_F(StatefulModelInstanceTest, PreprocessingFirstRequest) {
    ...
    currentBlobIrData.assign((float*)InferenceEngine::as<InferenceEngine::MemoryBlob>(stateCloneBlob)->rwmap(), ((float*)InferenceEngine::as<InferenceEngine::MemoryBlob>(stateCloneBlob)->rwmap()) + elementsCount);
...
TEST_F(StatefulModelInstanceTest, PostprocessingStartAndNoControl) {
    for (uint32_t sequenceControlInput : {ovms::NO_CONTROL_INPUT, ovms::SEQUENCE_START}) {
        ...
        sanityBlobIrData.assign((float*)InferenceEngine::as<InferenceEngine::MemoryBlob>(sanityBlob)->rwmap(), ((float*)InferenceEngine::as<InferenceEngine::MemoryBlob>(sanityBlob)->rwmap()) + elementsCount);
        ...
        sequenceBlobIrData.assign((float*)InferenceEngine::as<InferenceEngine::MemoryBlob>(chengedBlob)->rwmap(), ((float*)InferenceEngine::as<InferenceEngine::MemoryBlob>(chengedBlob)->rwmap()) + elementsCount);
