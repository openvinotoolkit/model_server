...
| [Qwen3 Coder](https://huggingface.co/OpenVINO/Qwen3-Coder-30B-A3B-Instruct-int4-ov) | qwen3coder | qwen3coder |  | ❌ Silent failure | ✅ Tools work | Minja lacks `from_json` filter → dumps raw JSON; Jinja has it. We pushed chat template long time ago which doesnt work with minja. **TODO: How to fix that?** |
| TODO [Phi4-mini](https://huggingface.co/microsoft/phi-4-mini-instruct) | *(undetected)* | |  | ⚠️ No tool support detected | ⚠️ No tool support detected | Analyzer doesn't find known tool markers; tools render if manually enabled |
