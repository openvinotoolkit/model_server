...
struct LLMTokenizeModelParameters {
    std::string modelName;
    int hello;
    int world;
    int bosToken;
...
class LLMTokenizeHttpTest : public V3HttpTest, public ::testing::WithParamInterface<LLMTokenizeModelParameters> {
protected:
    static std::unique_ptr<std::thread> t;
...
public:
    ...
    static void SetUpTestSuite() {
        std::string port = "9173";
        std::string configPath = getGenericFullPathForSrcTest("/ovms/src/test/llm/config.json");
    ...
    static std::vector<int> padTokens(std::vector<int> tokens, size_t length, int padToken, bool padLeft = true) {
        if (tokens.size() >= length) {
            tokens.resize(length);
            return tokens;
        }
        const size_t padCount = length - tokens.size();
        tokens.insert(padLeft ? tokens.begin() : tokens.end(), padCount, padToken);
        return tokens;
    }

    static std::vector<int> helloWorldTokens(const LLMTokenizeModelParameters& params) {
        return {params.hello, params.world};
    }
    ...
    static void AssertTokenizationResult(const std::string& response, const std::vector<int>& expectedTokens) {
        ...
        for (size_t i = 0; i < expectedTokens.size(); ++i) {
            auto actualToken = d["tokens"][(rapidjson::SizeType)i].GetInt();
            std::cout << actualToken << "\n";
            EXPECT_EQ(actualToken, expectedTokens[i]);
        }
    }
    ...
    static void AssertTokenizationResult(const std::string& response, const std::vector<std::vector<int>>& expectedTokensBatch) {
        ...
        for (size_t i = 0; i < expectedTokensBatch.size(); ++i) {
            ...
            for (size_t j = 0; j < expectedTokens.size(); ++j) {
                auto actualToken = d["tokens"][(rapidjson::SizeType)i][(rapidjson::SizeType)j].GetInt();
                std::cout << actualToken << "\n";
                EXPECT_EQ(actualToken, expectedTokens[j]);
            }
        }
    }
};

std::unique_ptr<std::thread> LLMTokenizeHttpTest::t;

TEST_P(LLMTokenizeHttpTest, tokenizePositive) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
    ...
    ASSERT_EQ(
        ...
        ovms::StatusCode::OK);
    AssertTokenizationResult(response, helloWorldTokens(params));
}

TEST_P(LLMTokenizeHttpTest, tokenizeNegativeMissingText) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
                "model": ")" + params.modelName +
...
TEST_P(LLMTokenizeHttpTest, tokenizeNegativeInvalidModel) {
    std::string requestBody = R"(
        {
...
TEST_P(LLMTokenizeHttpTest, tokenizePositiveMaxLenParam) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
...
TEST_P(LLMTokenizeHttpTest, tokenizePositivePadToMaxLenParam) {
    ...
    ASSERT_EQ(
        handler->dispatchToProcessor(endpointTokenize, requestBody, &response, comp, responseComponents, writer, multiPartParser),
        ovms::StatusCode::OK);
    AssertTokenizationResult(response, padTokens(helloWorldTokens(params), 100, params.padToken));
}

TEST_P(LLMTokenizeHttpTest, tokenizePositivePaddingSideLeft) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
    ...
    ASSERT_EQ(
        handler->dispatchToProcessor(endpointTokenize, requestBody, &response, comp, responseComponents, writer, multiPartParser),
        ovms::StatusCode::OK);
    AssertTokenizationResult(response, padTokens(helloWorldTokens(params), 100, params.padToken, true));
}

TEST_P(LLMTokenizeHttpTest, tokenizePositivePaddingSideRight) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
    ...
    ASSERT_EQ(
        handler->dispatchToProcessor(endpointTokenize, requestBody, &response, comp, responseComponents, writer, multiPartParser),
        ovms::StatusCode::OK);
    AssertTokenizationResult(response, padTokens(helloWorldTokens(params), 100, params.padToken, false));
}

TEST_P(LLMTokenizeHttpTest, tokenizeNegativeMaxLenParamAboveLimit) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
...
TEST_P(LLMTokenizeHttpTest, tokenizeNegativeInvalidPaddingSide) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
...
TEST_P(LLMTokenizeHttpTest, tokenizePositiveMaxLengthIgnored) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
    ...
    std::vector<int> expectedTokens = padTokens(helloWorldTokens(params), 513, params.padToken);
...
TEST_P(LLMTokenizeHttpTest, tokenizePositiveBatch) {
    auto params = GetParam();
    std::string requestBody = R"(
        {
            "model": ")" + params.modelName +
                              R"(",  
            "text": ["hello", "hello world", "hello world world world"]
        }
    )";
    std::vector<std::vector<int>> expectedTokens = {
        {params.hello},
        helloWorldTokens(params),
        {params.hello, params.world, params.world, params.world}};
    ASSERT_EQ(
        handler->dispatchToProcessor(endpointTokenize, requestBody, &response, comp, responseComponents, writer, multiPartParser),
        ovms::StatusCode::OK);
    AssertTokenizationResult(response, expectedTokens);
...
TEST_P(LLMTokenizeHttpTest, tokenizeBatchWithPadToMaxLen) {
    auto params = GetParam();
    ...
    std::vector<std::vector<int>> expectedTokens = {
        padTokens({params.hello}, maxLength, params.padToken),
        padTokens(helloWorldTokens(params), maxLength, params.padToken)};
    ASSERT_EQ(
        handler->dispatchToProcessor(endpointTokenize, requestBody, &response, comp, responseComponents, writer, multiPartParser),
        ovms::StatusCode::OK);
    AssertTokenizationResult(response, expectedTokens);
...
TEST_P(LLMTokenizeHttpTest, tokenizeEmptyNestedArray) {
    auto params = GetParam();
...
TEST_P(LLMTokenizeHttpTest, tokenizeMultipleEmptyNestedArrays) {
    auto params = GetParam();
    assertTokenizeWithInvalidTextReturnsError(handler.get(), params.modelName, "[[], [], []]", response, comp, responseComponents, writer, multiPartParser);
}

TEST_P(LLMTokenizeHttpTest, tokenizeMultipleEmptyNestedArraysAndOneNonEmpty) {
    auto params = GetParam();
...
TEST_P(LLMTokenizeHttpTest, tokenizeEmptyWithArrayMultipleLevelsOfNesting) {
    auto params = GetParam();
    assertTokenizeWithInvalidTextReturnsError(handler.get(), params.modelName, "[[[[[]]]]]", response, comp, responseComponents, writer, multiPartParser);
}

INSTANTIATE_TEST_SUITE_P(
    LLMTokenizeHttpTestInstances,
    LLMTokenizeHttpTest,
    ::testing::Values(
        LLMTokenizeModelParameters{"vlm_cb_regular", 31373, 995, 50256, 50256, 50256},
        LLMTokenizeModelParameters{"vlm_legacy_regular", 31373, 995, 50256, 50256, 50256},
        LLMTokenizeModelParameters{"lm_cb_regular", 31373, 995, 50256, 50256, 50256},
        LLMTokenizeModelParameters{"lm_legacy_regular", 31373, 995, 50256, 50256, 50256}),
    [](const ::testing::TestParamInfo<LLMTokenizeModelParameters>& info) {
        return info.param.modelName;
    });
