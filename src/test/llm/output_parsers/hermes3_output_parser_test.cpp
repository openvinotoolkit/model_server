...
TEST_F(Hermes3OutputParserTest, ParseToolCallWorksRegardlessOfSpecialTokens) {
    // OutputParser::parse() always operates with skip_special_tokens=true internally.
    // When user requests skip_special_tokens=false, the parser is bypassed at a higher level
    // (in parseOutputIfNeeded), so the parser itself always sees clean text.
...
TEST_F(Hermes3OutputParserTest, PlainContentWithNoToolsUsesDefaultDecode) {
    // When no tools are provided (toolsAvailable=false), the parser still decodes
    // with skip_special_tokens=true (default). The user's skip_special_tokens setting
    // is handled at a higher level by bypassing the parser entirely.
    std::string input = "Hello, world!";
    auto generatedTensor = hermes3Tokenizer->encode(input, ov::genai::add_special_tokens(true)).input_ids;
    std::vector<int64_t> generatedTokens(generatedTensor.data<int64_t>(), generatedTensor.data<int64_t>() + generatedTensor.get_size());

    ParsedOutput parsedOutput = outputParserWithRegularToolParsing->parse(generatedTokens, false);
    // Parser always strips special tokens internally
    EXPECT_EQ(parsedOutput.content, "Hello, world!");
}
