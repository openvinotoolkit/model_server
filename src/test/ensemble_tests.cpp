...
TEST_F(EnsembleFlowTestBinaryInput, EntryDemultiplexer) {
    ...
    std::vector<float> dat = {37.0, 28.0, 238.0, 37.0, 28.0, 238.0, 37.0, 28.0, 238.0, 37.0, 28.0, 238.0, 37.0, 28.0, 238.0};
    auto* d = (float*)response.outputs().at("pipeline_output").tensor_content().data();
    auto* e = dat.data();
...
static const char* pipelineSingleIncrement4DimOutputNHWC1x1EntryDemultiplexer__ = R"(
{
    "model_config_list": [
        {
            "config": {
...
TEST_F(EnsembleFlowTestBinaryInput, EntryDemultiplexer__) {
    std::string fileToReload = directoryPath + "/config.json";
    createConfigFileWithContent(pipelineSingleIncrement4DimOutputNHWC1x1EntryDemultiplexer__, fileToReload);
    ConstructorEnabledModelManager manager;
    std::unique_ptr<Pipeline> pipeline;

    //int batchSize = 5;
    prepareRequest({36.0, 27.0, 237.0, 36.0, 27.0, 237.0, 36.0, 27.0, 237.0, 36.0, 27.0, 237.0, 36.0, 27.0, 237.0},
        request, "pipeline_input", {5, 1, 3, 1, 1});
