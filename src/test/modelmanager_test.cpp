...
public:
    MockModelInstanceInStateWithConfig(ovms::ModelVersionState state, const ovms::ModelConfig& modelConfig, ov::runtime::Core& ieCore_2) :
        ModelInstance("UNUSED_NAME", UNUSED_MODEL_VERSION, ieCore_2) {
...
std::map<ovms::model_version_t, std::shared_ptr<ovms::ModelInstance>> getMockedModelVersionInstances(
    ...
    for (auto& state : states) {
        for (auto& version : initialVersionStates[state]) {
            modelVersions[version] = std::make_shared<MockModelInstanceInStateWithConfig>(state, modelConfig, ieCore_2);
        }
    }
...
TEST_F(ModelManagerVersionsReload, RetireOldAddNew) {
    ...
    std::map<ovms::model_version_t, std::shared_ptr<ovms::ModelInstance>> mockModelVersionInstances =
        getMockedModelVersionInstances(initialVersions, *ieCore_2);
...
TEST_F(ReloadAvailableModelDueToConfigChange, SameConfig_ExpectNoReloads) {
    mockModelVersionInstances = getMockedModelVersionInstances(initialVersions, *ieCore_2, config);
...
TEST_F(ReloadAvailableModelDueToConfigChange, ExpectReloadDueToMultipleInputShapeChange) {
    ...
    mockModelVersionInstances = getMockedModelVersionInstances(initialVersions, *ieCore_2, previouslyLoadedConfig);
...
public:
    MockModelInstanceFakeLoad(ov::runtime::Core& ieCore_2) :
        ModelInstance("UNUSED_NAME", UNUSED_MODEL_VERSION, ieCore_2) {}
...
public:
    ...
    std::shared_ptr<ovms::ModelInstance> modelInstanceFactory(const std::string& modelName, const ovms::model_version_t, ov::runtime::Core& ieCore_2) override {
        return std::make_shared<MockModelInstanceFakeLoad>(ieCore_2);
    }
};
...
public:
    ModelInstanceLoadedStuckInLoadingState(ov::runtime::Core& ieCore_2) :
        ModelInstance("UNUSED_NAME", UNUSED_MODEL_VERSION, ieCore_2) {}
...
public:
    ModelWithModelInstanceLoadedStuckInLoadingState(const std::string& name) :
        Model(name, false, nullptr) {}
    std::shared_ptr<ovms::ModelInstance> modelInstanceFactory(const std::string& modelName, const ovms::model_version_t, ov::runtime::Core& ieCore_2) override {
        return std::make_shared<ModelInstanceLoadedStuckInLoadingState>(ieCore_2);
    }
};
...
public:
    ModelInstanceLoadedWaitInLoadingState(ov::runtime::Core& ieCore_2, const uint modelInstanceLoadDelayInMilliseconds) :
        ModelInstance("UNUSED_NAME", UNUSED_MODEL_VERSION, ieCore_2),
...
public:
    ModelWithModelInstanceLoadedWaitInLoadingState(const std::string& name, const uint modelInstanceLoadDelayInMilliseconds) :
        Model(name, false, nullptr),
        modelInstanceLoadDelayInMilliseconds(modelInstanceLoadDelayInMilliseconds) {}
    std::shared_ptr<ovms::ModelInstance> modelInstanceFactory(const std::string&, const ovms::model_version_t, ov::runtime::Core& ieCore_2) override {
        return std::make_shared<ModelInstanceLoadedWaitInLoadingState>(ieCore_2, modelInstanceLoadDelayInMilliseconds);
    }
