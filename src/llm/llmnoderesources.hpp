...
public:
    ...
    T getProperty(const std::string& key) const {
        return properties.at(key).as<T>();
    };

    void setProperty(const std::string& key, const ov::Any& value) {
    ...
    absl::Status loadRequest(ov::AnyMap& executionContext, const ovms::HttpPayload& payload) {
    ...
    // Consider merging
    virtual absl::Status createApiHandler(ov::AnyMap& executionContext) = 0;
    
    virtual absl::Status parseRequest(ov::AnyMap& executionContext) = 0;
    // ---

    // Fill executionContext with data necessary to start the generation
    virtual absl::Status preparePipelineInput(ov::AnyMap& executionContext) = 0;
    ...
    virtual absl::Status schedulePipelineExecution(ov::AnyMap& executionContext) = 0;
    ...
    virtual absl::Status readCompleteExecutionResults(ov::AnyMap& executionContext) = 0;
    virtual absl::Status readPartialExecutionResults(ov::AnyMap& executionContext) = 0;
