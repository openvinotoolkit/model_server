...
absl::Status VisualLanguageModelLegacyServable::prepareCompleteResponse(std::shared_ptr<GenAiServableExecutionContext>& executionContext) {
    ...
    // By contract, readCompleteExecutionResults() has already awaited generation
    // completion and validated `success`, so the executor thread is no longer
    // touching the streamer state. The streamer callback appended the full
    // detokenized text into lastStreamerCallbackOutput during generation
    // (using operator+=); flushing the streamer here emits any trailing chunk
    // through the same callback.
    //
    // NOTE: textStreamer->end() invokes the callback synchronously on this
    // thread and the callback itself locks `mutex` - so end() must be called
    // without holding the lock to avoid a self-deadlock.
