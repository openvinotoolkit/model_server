...
class HttpLLMCalculator : public CalculatorBase {
    ...
    std::shared_ptr<BasicExecutionContext> executionContext;
...
public:
    ...
    absl::Status Open(CalculatorContext* cc) final {
        ...
        executionContext = nodeResources->createExecutionContext();
    ...
    absl::Status Process(CalculatorContext* cc) final {
        ...
        try {
            ...
            if (!cc->Inputs().Tag(INPUT_TAG_NAME).IsEmpty()) {
                ...
                if (status != absl::OkStatus())
                    return status;
                SPDLOG_LOGGER_DEBUG(llm_calculator_logger, "LLMCalculator  [Node: {}] API handler created successfully", cc->NodeName());
            ...
            if (!executionContext->apiHandler->isStream()) { // Unary scenario
                ...
                if (status != absl::OkStatus())
                    return status;
                SPDLOG_LOGGER_DEBUG(llm_calculator_logger, "LLMCalculator  [Node: {}] Received complete execution results, sending response", cc->NodeName());
                ...
                std::string& response = executionContext->response;
                if (!response.empty()) {
                    SPDLOG_LOGGER_DEBUG(llm_calculator_logger, "LLMCalculator  [Node: {}] Received partial execution results, sending response.", cc->NodeName());
