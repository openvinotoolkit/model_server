            ...
            if (member->value.IsString()) {
                ...
                if (member->name.GetString() == std::string("content") && member->value.IsArray()) {
                    ...
                    for (auto& v : member->value.GetArray()) {
                        if (!v.IsObject()) {
                            return absl::InvalidArgumentError("Invalid message structure - content array should contains objects");
                        }
...
std::string& OpenAIChatCompletionsHandler::getProcessedJson() {
    return request.processedJson;
}
std::vector<ov::Tensor> OpenAIChatCompletionsHandler::getImages() const {
