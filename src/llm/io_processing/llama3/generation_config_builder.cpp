...
void Llama3GenerationConfigBuilder::parseConfigFromRequest(const OpenAIChatCompletionsRequest& request) {
    ...
    std::string beginOfToolsString = "<|python_tag|>";
    ...
    // Set the structural tags config in the generation config
    if (config.structured_output_config) {
        ...
        config.structured_output_config = std::make_optional(std::move(structuredOutputConfig));
    }
}
