...
public:
    ...
    explicit GenerationConfigBuilder(ov::genai::GenerationConfig baseConfig, std::string toolParserName = "") {
        // Temporary solution for disabling guided generation on demand. TODO: switch to per model configuration
        bool disableGuidedGeneration = false;
        const char* envVar = std::getenv("OVMS_DISABLE_GUIDED_GENERATION");
        if (envVar != nullptr && std::string(envVar) == "1") {
            disableGuidedGeneration = true;
        }

        if (disableGuidedGeneration) {
