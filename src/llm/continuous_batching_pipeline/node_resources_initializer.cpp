...
void ContinuousBatchingNodeResourcesInitializer::loadTextProcessor(std::shared_ptr<LLMNodeResources>& nodeResources, const std::string& chatTemplateDirectory) {
    ...
    try {
        ...
        std::shared_ptr<TextProcessor> textProcessor = std::make_shared<TextProcessor>();
        ...
        nodeResources->setProperty("text_processor", textProcessor);
...
Status ContinuousBatchingNodeResourcesInitializer::initialize(std::shared_ptr<LLMNodeResources>& nodeResources, const mediapipe::LLMCalculatorOptions& nodeOptions, std::string graphPath) {
    ...
    schedulerConfig.max_num_batched_tokens = mainPipelineConfig.max_num_batched_tokens();
    schedulerConfig.cache_size = mainPipelineConfig.cache_size();
    schedulerConfig.dynamic_split_fuse = mainPipelineConfig.dynamic_split_fuse();
    schedulerConfig.max_num_seqs = mainPipelineConfig.max_num_seqs();
    schedulerConfig.enable_prefix_caching = mainPipelineConfig.enable_prefix_caching();

    std::string device = mainPipelineConfig.device();
    ...
    if (continousBatchingPipelineConfig.has_draft_pipeline_config()) {
        ...
        pluginConfig.insert(draftPipeline);
        isSpeculativePipeline = true;
    }
    
    status = JsonParser::parsePluginConfig(mainPipelineConfig.plugin_config(), pluginConfig);
    if (!status.ok()) {
        SPDLOG_ERROR("Error during llm node plugin_config option parsing to JSON: {}", mainPipelineConfig.plugin_config());
        return status;
    }

    ov::AnyMap tokenizerPluginConfig = {{"PERFORMANCE_HINT", "THROUGHPUT"}};
    std::shared_ptr<ov::genai::ContinuousBatchingPipeline> pipeline;
    try {
        pipeline = std::make_shared<ov::genai::ContinuousBatchingPipeline>(getBasePath(), schedulerConfig, device, pluginConfig, tokenizerPluginConfig);
    ...
    loadTextProcessor(nodeResources, getBasePath());

    nodeResources = std::make_shared<ContinuousBatchingNodeResources>();
    nodeResources->setProperty("models_path", getBasePath());
    nodeResources->setProperty("scheduler_config", schedulerConfig);
    nodeResources->setProperty("device", device);
    nodeResources->setProperty("plugin_config", pluginConfig);
    nodeResources->setProperty("tokenizer_plugin_config", tokenizerPluginConfig);
    nodeResources->setProperty("pipeline", std::move(pipeline));
    nodeResources->setProperty("max_tokens_limit", mainPipelineConfig.max_tokens_limit());
    nodeResources->setProperty("best_of_limit",mainPipelineConfig.best_of_limit());
    nodeResources->setProperty("is_speculative_pipeline", isSpeculativePipeline);
...
Status ContinuousBatchingNodeResourcesInitializer::initializeLegacy(std::shared_ptr<LLMNodeResources>& nodeResources, const mediapipe::LLMCalculatorOptions& nodeOptions, std::string graphPath) {
    ...
    ov::genai::SchedulerConfig schedulerConfig;
    schedulerConfig.max_num_batched_tokens = nodeOptions.max_num_batched_tokens();
    schedulerConfig.cache_size = nodeOptions.cache_size();
    schedulerConfig.dynamic_split_fuse = nodeOptions.dynamic_split_fuse();
    schedulerConfig.max_num_seqs = nodeOptions.max_num_seqs();
    schedulerConfig.enable_prefix_caching = nodeOptions.enable_prefix_caching();
    ...
    status = JsonParser::parsePluginConfig(nodeOptions.plugin_config(), pluginConfig);
    if (!status.ok()) {
    ...
    nodeResources->setProperty("tokenizer_plugin_config", tokenizerPluginConfig);
    nodeResources->setProperty("pipeline", std::move(pipeline));
    nodeResources->setProperty("max_tokens_limit", nodeOptions.max_tokens_limit());
    nodeResources->setProperty("best_of_limit",nodeOptions.best_of_limit());
    nodeResources->setProperty("is_speculative_pipeline", isSpeculativePipeline);
