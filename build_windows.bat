...
set "buildCommand=bazel %bazelStartupCmd% build --config=windows --jobs=%NUMBER_OF_PROCESSORS% --verbose_failures --define CLOUD_DISABLE=1 --define MEDIAPIPE_DISABLE=0 --define PYTHON_DISABLE=1 //src:ovms 2>&1 | tee build.log"
set "buildTestCommand=bazel %bazelStartupCmd% build --config=windows --jobs=%NUMBER_OF_PROCESSORS% --verbose_failures --define CLOUD_DISABLE=1 --define MEDIAPIPE_DISABLE=0 --define PYTHON_DISABLE=1 //src:ovms_test 2>&1 | tee build_test.log"
set "runTest=bazel-bin\src\ovms_test.exe --gtest_filter=-OvmsConfigTest 2>&1 | tee test.log"
