...
from tests.functional.config import is_nginx_mtls, models_path, xdist_workers
...
class ModelInfo:
    ...
    def prepare_resources(self, base_location):
        ...
        if not self.is_on_cloud and self.model_path_on_host is not None:
            if self.copy_all_model_versions:
                src_model_path = Path(self.model_path_on_host).parent
                target_model_dir = Path(resource_destination, src_model_path.name)
            else:
