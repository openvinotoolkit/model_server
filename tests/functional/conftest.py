...
def get_product_build_number_from_container():
    client = docker.from_env()
    output = client.containers.run(image=image,
                                   entrypoint=["/ovms/bin/ovms", "--version"])
    output = output.decode("utf-8")
    _ovms_version = re.search('OpenVINO Model Server (.+)\n', output, re.MULTILINE).group(1)
    _open_vino_version = re.search('OpenVINO backend (.+)\n', output, re.MULTILINE).group(1)

    return f"{_ovms_version}_OV{_open_vino_version}"
...
def extra_json_environment(request):
    ...
    request.config._json_environment.append(('version', get_product_build_number_from_container()))
    return
