...
class OvmsPythonModel:
    ...
    def execute(self, inputs: list):
        ...
        for i in range(len(text)):
            ...
            alternative_output_data = func(self, text=self.alternative_input_text, counter=i)
            outputs_list = [Tensor(output, output_data) for output in self.output_names]
            outputs_list += [Tensor(output, output_data) for output in self.output_names]
            outputs_list += [Tensor(output, alternative_output_data) for output in self.output_names]
            yield outputs_list
