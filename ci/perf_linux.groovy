    ...
    stages {
        ...
        stage('Agentic Latency') {
            ...
            steps {
                ...
                sh "source .venv/bin/activate && pip install -r vllm/benchmarks/multi-turn/requirements.txt"
                sh "sed -i -e 's/if not os.path.exists(args.model)/if 1 == 0/g' vllm/benchmarks/multi-turn/benchmark_serving_multi_turn.py"
                sh "test -f pg1184.txt || wget https://www.gutenberg.org/ebooks/1184.txt.utf-8 && mv 1184.txt.utf-8 pg1184.txt"
                sh "source .venv/bin/activate && python vllm/benchmarks/multi-turn/benchmark_serving_multi_turn.py -m ${params.MODEL} --url http://localhost:9000/v3 -i vllm/benchmarks/multi_turn/generate_multi_turn.json --served-model-name ${params.MODEL} --num-clients 1 -n 20 > results_agentic_latency.txt"
        ...
        stage('Agentic Accuracy') {
            ...
            steps {
                ...
                sh "source .venv/bin/activate && pip install -e ./gorilla/berkeley-function-call-leaderboard"
                sh "echo Running agentic accuracy test"
                sh "export OPENAI_BASE_URL=http://localhost:8000/v3 && source .venv/bin/activate && bfcl generate --model ovms-model --test-category simple --temperature 0.0 --num-threads 100 -o --result-dir bfcl_results && bfcl evaluate --model ovms-model --result-dir bfcl_results --score_dir bfcl_scores"
