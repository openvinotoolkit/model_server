          ...
          parallel {
            stage("Build linux") {
              ...
              steps {
                      ...
                      sh "make ovms_builder_image RUN_TESTS=0 OPTIMIZE_BUILDING_TESTS=1 OV_USE_BINARY=0 BASE_OS=ubuntu22 OVMS_CPP_IMAGE_TAG=${shortCommit} BUILD_IMAGE=openvino/model_server-build:${shortCommit}"
                    }
            }
            stage('Build windows') {
              agent {
        ...
        stage("Release image and tests in parallel") {
          ...
          parallel {
            stage("Run unit tests") {
              ...
              try {
                  sh "make run_unit_tests TEST_LLM_PATH=${HOME}/ovms_models/llm_models_ovms/INT8 BASE_OS=ubuntu22 OVMS_CPP_IMAGE_TAG=${shortCommit}"
              }
              finally {
            ...
            stage("Internal tests") {
              ...
              steps {
                sh "make release_image RUN_TESTS=0 OV_USE_BINARY=0 BASE_OS=ubuntu22 OVMS_CPP_IMAGE_TAG=${shortCommit} BUILD_IMAGE=openvino/model_server-build:${shortCommit}"
                ...
                script {
                  dir ('internal_tests'){ 
                    ...
                    sh 'pwd'
                    sh "make create-venv && TT_ON_COMMIT_TESTS=True TT_XDIST_WORKERS=10 TT_BASE_OS=ubuntu22 TT_OVMS_IMAGE_NAME=openvino/model_server:${shortCommit} TT_OVMS_IMAGE_LOCAL=True make tests"
                  }
                }
              }            
            }
