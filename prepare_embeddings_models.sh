...
echo "Downloading embeddings testing models to directory $1"
if [ "$2" = "docker" ]; then
...
optimum-cli export openvino --model "$MODEL" --task feature-extraction "$1/${MODEL}_embeddings/1"
convert_tokenizer -o "$1/${MODEL}_tokenizer/1" --skip-special-tokens "$MODEL"
rm "$1/${MODEL}_embeddings/1/openvino_tokenizer.xml"
rm "$1/${MODEL}_embeddings/1/openvino_tokenizer.bin"
