...
def load_tf_image(file_path):
    ...
    with open(file_path, mode='rb') as image_file:
        ...
        image = preprocess_binary_image(image_bytes,reverse_input_channels=True)
    return image
...
def interpret_array(output, img_out):
    ...
    for i in range(0, 199):  # there is returned 200 detections for each image in the batch
        ...
        # box coordinates are proportional to the image size
            ...
            img_out = cv2.rectangle(img_out,(x_min_s,y_min_s),(x_max_s,y_max_s),(0,0,255),1)
...
def interpret_ams(json_txt, img_out):
    ...
    for detection in detections:
        ...
        box = detection["box"]
        print("l:", box["l"]," t:" ,box["t"]," w:",box["w"], " h:", box["h"])
...
print("loading image via opencv", i1.shape, " sum:", np.sum(i1), " dtype:", i1.dtype)

i2 = load_tf_image(test_image)
print("loading image via tensorflow", i2.shape, " sum:", np.sum(i2), " dtype:", i2.dtype)

diff = np.absolute(i1.astype(np.float32) - i2.astype(np.float32)).transpose(1,2,0)
print("diff", np.sum( np.absolute(i1.astype(np.float32) - i2.astype(np.float32)) ) )

print("Average normalized difference in volume:",
      np.sum(np.absolute(i1.astype(np.float32) - i2.astype(np.float32)))/np.sum(i1),
      "should be < 0.02")
