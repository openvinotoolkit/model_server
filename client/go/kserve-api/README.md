...
```Bash
docker run --rm -d -v $(pwd)/src/test/dummy:/models -p 9000:9000 openvino/model_server:2026.1 --model_name dummy --model_path /models --port 9000
```
...
```Bash
docker run --rm -d -v $(pwd)/models:/models -p 9000:9000 openvino/model_server:2026.1 --model_name resnet --model_path /models/resnet --port 9000 --layout NHWC:NCHW --plugin_config '{"PERFORMANCE_HINT":"LATENCY"}'
```
