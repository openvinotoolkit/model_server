...
import inference.GrpcPredictV2.ModelReadyRequest;
import inference.GrpcPredictV2.ModelReadyResponse;
import io.grpc.ManagedChannel;
import io.grpc.ManagedChannelBuilder;

public class grpc_model_ready {
	...
	public static void main(String[] args) {
		...
		if (cmd.hasOption("h")) {
			...
			formatter.printHelp("grpc_model_ready [OPTION]...", opt);
			System.exit(0);
		}
		...
		ModelReadyRequest.Builder request = ModelReadyRequest.newBuilder();
		request.setName(cmd.getOptionValue("n", "dummy"));
		request.setVersion(cmd.getOptionValue("v", ""));

		ModelReadyResponse response = grpc_stub.modelReady(request.build());

		// Get the response outputs
		System.out.format("Model Ready: %b\n", response.getReady());
		channel.shutdownNow();
