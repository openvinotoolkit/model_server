...
def test_get_model_status_valid(mocker, valid_http_serving_client_min,
    ...
    mock_requests_get = mocker.patch('requests.get', return_value=raw_response)
...
@pytest.mark.parametrize("expected_message", [
    ...
    ("HTTPConnectionPool(host='localhost', port=54000): Max retries exceeded with url: /v1/models/resnet/versions/1 "
     "(Caused by NewConnectionError('<urllib3.connection.HTTPConnection object at 0x7ff21a41f370>: Failed to establish a new connection: "
...
def test_get_model_status_invalid_http(mocker, valid_http_serving_client_min,
    ...
    mock_check_request = mocker.patch('ovmsclient.tfs_compat.http.serving_client'
                                      '.HttpClient._check_model_status_request')
    mock_requests_get =\
        mocker.patch('requests.get',side_effect=requests.exceptions.ConnectionError(expected_message))

    with pytest.raises(ConnectionError) as e_info:
        valid_http_serving_client_min.get_model_status(model_status_request)

    assert str(e_info.value) == "There was an error during sending ModelStatusRequest. Http exited with:\n" + expected_message
    assert mock_check_request.call_count == 1
    assert mock_requests_get.call_count == 1
