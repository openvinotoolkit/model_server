...
from utils.utils import load_image, read_paths, get_single_metadata
...
parser.add_argument('--model_name', default='vehicle-detection',
                    help='Pipeline name to query. default: detection')
parser.add_argument('--model_version', default=0, type=int,
                    help='Model version to query. Lists all versions if omitted')
parser.add_argument('--output_save_path', required=False, default='./result',
...
for path in paths:
    ...
    vehicle_postprocess(response, path, output_name, output_save_path)
