...
class OVStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        print("getting initial random noise", latents.shape, meta)
    ...
    def prepare_latents(self, image:PIL.Image.Image = None, latent_timestep:torch.Tensor = None):
        ...
        Parameters:
            image (PIL.Image.Image, *optional*, None):
                Input image for generation, if not provided random noise will be used as starting point
            latent_timestep (torch.Tensor, *optional*, None):
                Predicted by scheduler initial step for image generation, required for latent image mixing with noise
        Returns:
            latents (np.ndarray):
        ...
        print("mean and logvar and std", mean.shape, logvar.shape, std.shape)
        latents = (mean + std * np.random.randn(*mean.shape)) * 0.18215
        latents = self.scheduler.add_noise(torch.from_numpy(latents), torch.from_numpy(noise), latent_timestep).numpy()
        print("new latents", latents.shape)
        return latents, meta
