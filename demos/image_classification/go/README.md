...
```bash
docker run -d --rm -p 9000:9000  -v ${PWD}/models:/models openvino/model_server:2026.1 --model_name resnet --model_path /models/resnet --port 9000 --layout NHWC:NCHW --mean "[123.675,116.28,103.53]" --scale "[58.395,57.12,57.375]" --shape "(1,224,224,3)" --color_format BGR:RGB --precision uint8:fp32
```

**Note** Layout for downloaded resnet model is changed to NHWC. It ensures that the model will accept binary input generated by the client. See [binary inputs](../../../docs/binary_input.md) doc if you want to learn more about this feature.
