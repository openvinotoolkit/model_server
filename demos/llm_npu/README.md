...
```bash
mkdir -p models
docker run -d --rm -u $(id -u):$(id -g) -v $(pwd)/models:/models:rw openvino/model_server:latest-gpu --pull --source_model OpenVINO/Qwen3-8B-int4-cw-ov --model_repository_path /models --target_device NPU --task text_generation --cache_dir /models/.ov_cache --enable_prefix_caching true --max_prompt_len 2000
...
```bat
mkdir models
ovms.exe --pull --source_model OpenVINO/Qwen3-8B-int4-cw-ov --model_repository_path models --target_device NPU --task text_generation --cache_dir .ov_cache --enable_prefix_caching true --max_prompt_len 2000 
