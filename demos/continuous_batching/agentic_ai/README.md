...
```
> **Note:**: Use `--pipeline_type LM` for export and `--target device GPU` for deployment. Using weekly release, it is recommended to drop `--pipeline_type LM` to enable continuous batching.
