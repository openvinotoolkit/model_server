...
class ServingClient {
 public:
      ...
      for (iter = map_outputs.begin(); iter != map_outputs.end(); ++iter) {
        ...
        if (converted) {
          std::cout << "the result tensor[" << output_index
                    << "] is:" << std::endl
                    << tensor.SummarizeValue(10, true)
                    << "\nMost probable label: "
                    << this->getMostProbableLabel(tensor)
                    << "\n" << tensor.shape().DebugString()
                    << "\n" << tensor.NumElements() << std::endl;
        } else {
          std::cout << "the result tensor[" << output_index
  ...
  tensorflow::int64 getMostProbableLabel(const tensorflow::Tensor& tensor) const {
    float top_confidence = 0;
    tensorflow::int64 top_label = -1;
    for (tensorflow::int64 i = 0; i < tensor.NumElements(); i++) {
      float confidence = ((float*)tensor.data())[i];
      if (top_label == -1 || top_confidence < confidence) {
        top_label = i;
        top_confidence = confidence;
      }
    }
    std::cout << "Top confidence: " << top_confidence << std::endl;
    return top_label;
  }
