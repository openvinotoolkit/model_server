...
$KALDI_PATH/src/bin/latgen-faster-mapped --max-active=7000 --max-mem=50000000 --beam=13 --lattice-beam=6.0 --acoustic-scale=1.0 --allow-partial=true --word-symbol-table=words.txt $ASPIRE_PATH/exp/chain/tdnn_7b/final.mdl HCLG.fst ark:/opt/data/scores.ark ark:-| $KALDI_PATH/src/latbin/lattice-scale --inv-acoustic-scale=13 ark:- ark:- | $KALDI_PATH/src/latbin/lattice-best-path --word-symbol-table=words.txt ark:- ark,t:-  > /opt/data/out.txt
cat $DATA_PATH/out.txt | $ASPIRE_PATH/utils/int2sym.pl -f 2- words.txt | tee $DATA_PATH/$1.txt
