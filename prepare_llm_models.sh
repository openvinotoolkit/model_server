...
MODELS=("$CB_MODEL" "$EMBEDDING_MODEL" "$RERANK_MODEL" "$VLM_MODEL" "$QWEN3_MODEL" "$LLAMA3_MODEL" "$HERMES3_MODEL" "$EMBEDDING_MODEL/ov" "$RERANK_MODEL/ov")
...
if [ -d "$1/$RERANK_MODEL/ov" ]; then
  ...
  python3 demos/common/export_models/export_model.py embeddings_ov --source_model "$RERANK_MODEL" --weight-format int8 --model_repository_path $1
fi
