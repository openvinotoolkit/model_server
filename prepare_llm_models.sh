...
if [ -f "$1/$DEEPSEEK_MODEL/$TOKENIZER_FILE" ]; then
  echo "Models file $1/$DEEPSEEK_MODEL/$TOKENIZER_FILE exists. Skipping downloading models."
else
  mkdir -p $1/$DEEPSEEK_MODEL
  convert_tokenizer $DEEPSEEK_MODEL --with_detokenizer -o $1/$DEEPSEEK_MODEL
fi
if [ ! -f "$1/$DEEPSEEK_MODEL/$TOKENIZER_FILE" ]; then
  echo "[ERROR] Models file $1/$DEEPSEEK_MODEL/$TOKENIZER_FILE does not exist."
  exit 1
fi
