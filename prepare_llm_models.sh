...
if [ -d "$1/$EMBEDDING_MODEL/ov" ]; then
  ...
  python3 demos/common/export_models/export_model.py embeddings_ov --source_model "$EMBEDDING_MODEL" --weight-format int8 --model_repository_path $1 --name_in_config $RERANK_MODEL/ov
fi
...
if [ -d "$1/$RERANK_MODEL/ov" ]; then
  ...
  python3 demos/common/export_models/export_model.py rerank_ov --source_model "$RERANK_MODEL" --weight-format int8 --model_repository_path $1 --name_in_config $RERANK_MODEL/ov
fi
