...
```
curl https://raw.githubusercontent.com/openvinotoolkit/model_server/refs/heads/releases/2026/4/extras/kserve/kserve-openvino.yaml -O
sed -i 's/openvino\/model_server:replace/openvino\/model_server:2026.4.0/' kserve-openvino.yaml
kubectl apply -f kserve-openvino.yaml
```
Note: Alternatively use the official image tag `2026.4.0` to deploy smaller image with support for CPU only or `2026.4.0-gpu` with support for GPU and CPU.
