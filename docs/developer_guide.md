...
5. From the container, run a single unit test (choose distro `ubuntu` or `redhat` depending on the image type):
	```bash
	bazel test --//:distro=ubuntu --config=mp_on_py_on --test_env=https_proxy=$https_proxy --test_summary=detailed --test_output=all --test_filter='ModelVersionStatus.*' //src:ovms_test
	```
