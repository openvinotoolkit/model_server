...
- guarding binary image inputs against decompression amplification. A small compressed image can decode into a much larger raw buffer, and for vision language models it can amplify further inside the vision encoder, so the decoded image size is bounded **by pixel count** both before and after decoding. The following limits apply:
    - `--max_image_decode_pixels` - OVMS in-process pixel budget (default `67108864`, matching `OPENCV_IO_MAX_IMAGE_PIXELS`) enforced on every request across all images. Images whose header-declared pixel count exceeds it are rejected before decoding.
    - `--allow_unestimatable_image_formats` - when `false` (default), images whose decoded pixel count cannot be estimated are rejected; set `true` only if you must accept such formats.
    - `OPENCV_IO_MAX_IMAGE_PIXELS` - OpenCV's own header-stage guard, read once at process startup and **separate** from the CLI parameter above (changing `--max_image_decode_pixels` does not change OpenCV's internal limit). It must be set before OVMS starts. The provided container images set it by default; on bare-metal set it yourself before launching OVMS.
