...
| `"layout" `| `json / string` | `layout` is optional argument which allows to change the layout of model input and output tensors. To add the transposition step, specify tensor layout and native model layout separated by `:` (colon). Example: `NHWC:NCHW` means that user will send input data in `NHWC` layout but model is in `NCHW` layout.<br><br>When specified with single string value - layout change is only applied to single model input. To change multiple model inputs or outputs, you can specify json object with mapping, such as: `{"input1":"NHWC:NCHW","input2":"HWN:NHW","output1":"CN:NC"}`.<br><br>If not specified, layout is inherited from model. ||
