...
| `sequence_cleaner_poll_wait_minutes` | `integer` | Time interval (in minutes) between next sequence cleaner scans. Sequences of the models that are subjects to idle sequence cleanup that have been inactive since the last scan are removed. Zero value disables sequence cleaner. See [idle sequence cleanup](stateful_models.md). Note that it also calls [`malloc_trim()`](https://man7.org/linux/man-pages/man3/malloc_trim.3.html) which attempts to release free memory from the heap. |
