...
| lora_weights | ✅ | ❌ | object (optional) | A JSON object mapping LoRA adapter aliases to weight overrides for the active adapter(s). Adapter selection is driven by the `model` field (model name routing). This field only overrides the weights of adapters already activated by model name routing — it does not independently select adapters. Aliases must match adapters registered via `--source_loras` at server start. Example: `{"pokemon": 0.5}` (when `model` is `"pokemon"` or a composite containing it). See [LoRA Adapters documentation](./image_generation/reference.md#lora-adapters). |
