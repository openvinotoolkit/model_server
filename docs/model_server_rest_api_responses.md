...
| stream_options.include_usage | ✅ | ❌ | bool (optional) | If set, usage statistics are tracked. Note: for responses endpoint usage is always included in the `response.completed` event. |
...
| reasoning | ⚠️ | ✅ | object (optional) | Configuration for reasoning/thinking mode. The `effort` field accepts `"low"`, `"medium"`, or `"high"` — any value enables thinking mode (`enable_thinking: true` is injected into chat template kwargs). The `summary` field is accepted but ignored. If `chat_template_kwargs.enable_thinking` is also set explicitly, the explicit value takes precedence. |
| chat_template_kwargs | ✅ | ❌ | object (optional) | Additional keyword arguments passed to the chat template. For example, `{"enable_thinking": true}` enables reasoning output for models that support it. |
