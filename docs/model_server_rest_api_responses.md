...
| reasoning | ⚠️ | ✅ | object (optional) | Configuration for reasoning/thinking mode. The `effort` field accepts one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`. Injects `reasoning_effort` (the literal value) and `enable_thinking` (`false` only for `none`, `true` otherwise) into `chat_template_kwargs`. Also injects `reasoning_strength`, mapped onto `low`/`medium`/`high`/`xhigh` for templates (e.g. Muse/Onyx) that only understand that reduced set (`none`/`minimal` → `low`, `max` → `xhigh`). None of these are injected if the request already sets the corresponding key explicitly. The `summary` field is accepted but ignored. |
| chat_template_kwargs | ✅ | ❌ | object (optional) | Additional keyword arguments passed to the chat template. When `reasoning` is also provided, `reasoning_effort`, `reasoning_strength` and `enable_thinking` are merged into these kwargs (explicit `chat_template_kwargs` keys always take precedence). Also accepts `add_generation_prompt` (bool, default: `true`) — whether to append the chat template's generation prompt (the marker that signals the model to start a new assistant turn). Set to `false` to render the conversation without a trailing generation prompt, e.g. `{"add_generation_prompt": false}` — useful for assistant prefill where the final `assistant` message should be continued rather than treated as a completed turn. Applies to both the Python-Jinja and MINJA chat template paths. |
