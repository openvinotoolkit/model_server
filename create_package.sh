...
find /ovms/bazel-out/k8-*/bin -iname '*.so*' ! -type d ! -name "*params" ! -name "lib_node_*" ! -path "*test_python_binding*" -exec cp -v {} /ovms_release/lib/ \;
...
if ! [[ $debug_bazel_flags == *"PYTHON_DISABLE=1"* ]]; then find /opt/intel/openvino/python/ -iname '*.so*' -exec cp -vP {} /ovms_release/lib/ \; ; fi ;
if ! [[ $debug_bazel_flags == *"PYTHON_DISABLE=1"* ]]; then cp -r /opt/intel/openvino/python/openvino-2023.2.dist-info /ovms_release/lib/ \; ; fi ;
