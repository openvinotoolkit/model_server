#!/bin/bash -x
openssl req -x509 -nodes -days 7 -newkey rsa:2048 -keyout server.key -out server.pem -subj "/C=US/CN=www.example.com"

openssl genrsa -out client_cert_ca.key 2048
openssl req -x509 -new -nodes -key client_cert_ca.key -sha256 -days 1024 -out client_cert_ca.pem -subj "/C=US/CN=client-cert-root-ca.example.com"
...
docker run --rm -ti \
	...
	--model_path /models/face-detection --model_name face-detection --port 9000 --rest_port 5556 --log_level DEBUG --shape auto
